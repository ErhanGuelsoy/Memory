...
class MemoryGame {
    ...
    private buildCards(): Card[] {
        ...
        for (let i = 1; i <= this.totalPairs; i++) {
            const image = `/public/assets/images/Code vibes card ${i}.png`;
    ...
    /**
     * Shuffles and renders all cards.
    ...
    /**
     * Creates the HTML markup for a card.
     * @param card Card object.
     ...
     * @param element Card HTML element.
     */
     ...
     * @returns True when all selected objects exist.
     */
    ...
    /**
     * Removes the flipped class from selected cards.
     */
    ...
    private showWinnerPopup(): void {
        const popup = document.querySelector<HTMLElement>("#winnerPopup");
        if (!popup) return;
    ...
    private updateWinnerPopup(): void {
        ...
        const blueWinner = document.querySelector<HTMLElement>("#blueWinner");

        if (!orangeWinner || !blueWinner) return;
     ...
     * @param scoreSelector Score element selector.
    ...
    private setupExitPopup(): void {
        ...
        const backToGame = document.querySelector<HTMLElement>("#backToGame");
    ...
    private handlePopupClick(
        ...
        if (popup && target instanceof Node && !popup.contains(target)) {
            this.closeExitPopup(exitPopup);
        }
    }
